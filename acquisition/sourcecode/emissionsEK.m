function [Power, Emissions] = emissionsEK(Power, EmissionsCategory, Emissionsdatabase, Emissions, zone)


EFSourcelist = {'EcoInvent' 'IPCC'} ;
if isa(Power.(zone).TSO.bytech, 'struct')
    temptable = struct2table(Power.(zone).TSO.bytech) ;
else
    temptable = table(0,'VariableNames',{'biomass'}) ;
end
for iEFSource = 1:length(EFSourcelist)
    EFSource = EFSourcelist{iEFSource} ;
    if isa(Power.(zone).TSO.bytech, "double")
        Emissions.(zone).TSO.(EFSource).intensityprod = 0 ;
        continue ;
    else
        alltech = fieldnames(Power.(zone).TSO.bytech) ;
    end
    alltech = fieldnames(Power.(zone).TSO.bytech) ;
    hydro               = 0 ;
    solar               = 0 ;
    ...
    for itechname = 1:length(alltech)
        ...
        switch technameinTSO
            case {'hydro', 'hydrodam', 'hydropumped'}
                if isfield(Power.(zone).TSO.bytech,'hydro') && isfield(Power.FR.TSO.bytech,'hydrodam') && isfield(Power.FR.TSO.bytech,'hydropumped')
                    hydro   = Power.(zone).TSO.bytech.hydro   * extractdata('hydro_runof', 'FR', EmissionsCategory, Emissionsdatabase.(EFSource)) + ...
                            Power.(zone).TSO.bytech.hydrodam   * extractdata('hydro_reservoir', 'FR', EmissionsCategory, Emissionsdatabase.(EFSource)) + ...    
                            Power.(zone).TSO.bytech.hydropumped   * extractdata('hydro_pumped', 'FR', EmissionsCategory, Emissionsdatabase.(EFSource)) ;
